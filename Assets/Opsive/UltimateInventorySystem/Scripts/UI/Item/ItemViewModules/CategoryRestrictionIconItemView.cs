...
namespace Opsive.UltimateInventorySystem.UI.Item.ItemViewModules
{
    ...
    public class CategoryRestrictionIconItemView : ItemViewModule
    {
        ...
        {
            var categoryRestriction = View.ViewSlot.gameObject.GetCachedComponent<ItemViewSlotCategoryRestriction>();
            ...
            m_Icon.enabled = true;
            if (categoryRestriction.ItemCategory.TryGetCategoryAttributeValue<Sprite>(m_CategoryIconAttributeName, out var icon)) {
                m_Icon.sprite = icon;
            } else {
