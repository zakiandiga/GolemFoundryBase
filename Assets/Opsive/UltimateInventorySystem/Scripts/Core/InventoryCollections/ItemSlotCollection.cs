...
namespace Opsive.UltimateInventorySystem.Core.InventoryCollections
{
    ...
    public class ItemSlotCollection : ItemCollection, IDatabaseSwitcher
    {
        ...
        {
            var result = AddItemCondition(itemInfo);
            if (result.HasValue == false) { return (0, itemInfo.Item, this); }

            var item = result.Value.Item;
            var amount = result.Value.Amount;

            if (!item.IsUnique || amount <= 1) { return AddItem(itemInfo, GetTargetSlotIndex(item)); }

            var originalResult = AddItem((item, 1, itemInfo), stackTarget);

            for (int i = 1; i < amount; i++) { AddItem((DuplicateItem(item), 1, itemInfo), stackTarget); }

            return (amount, originalResult);
        }
        ...
        public ItemInfo AddItem(ItemInfo itemInfo, int slotIndex)
        {
            ...
            var setItemInfo = SetItemAmount(item, slotIndex, amount + currentAmount, true);
            if (setItemInfo.HasValue) { return setItemInfo.Value; } else { return (0, itemInfo.Item, this); }
        }
        ...
        private ItemInfo? SetItemAmount(Item item, int slotIndex, int amount, bool removePreviousItem)
        {
            var itemSlot = m_ItemSlotSet.GetSlot(slotIndex);
            if (!itemSlot.HasValue) {
                return null;
            ...
            if (currentStack != null) {
                if (item.StackableEquivalentTo(currentStack.Item)) {
            ...
            var itemToAdd = new ItemInfo((item, amount), null);
        ...
        public ItemInfo RemoveItem(int slotIndex, int amount = -1)
        {
            ...
            var removed = RemoveInternal((itemToRemove.Item, amountToRemove, this));
