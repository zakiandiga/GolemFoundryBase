    ...
    public abstract class AttributeBase //: IEquatable<AttributeBase>
    {
        ...
        public (bool, string) ExpressionRegexReplaceAttributes(string modifyExpression, AttributeBase relativeAttribute, IReadOnlyDictionary<string, AttributeBase> otherAttributes, out string finalExpression)
        {
            ...
            if (overrideValue != null) {
                finalExpression = Regex.Replace(finalExpression, c_AbsoluteOverridePattern, overrideValue.ToString());
            }

            var inheritedValue = GetInheritValueAsObject();
            if (inheritedValue != null) {
                finalExpression = Regex.Replace(finalExpression, c_AbsoluteInheritedPattern, inheritedValue.ToString());
            }
            ...
            if (relativeOverrideValue != null) {
                ...
                finalExpression = Regex.Replace(finalExpression, c_RelativeOverridePattern, m => string.Format("{0}{1}", m.Groups[1].Value, relativeOverrideValue.ToString()));
            }
            ...
            if (Regex.Match(finalExpression, c_RelativeInheritedPattern).Success == true) {
                if (relativeAttribute.VariantType == VariantType.Inherit || relativeAttribute.VariantType == VariantType.Modify) {
                    if (inheritedValue != null) {
                        finalExpression = Regex.Replace(finalExpression, c_RelativeInheritedPattern, m => string.Format("{0}{1}", m.Groups[1].Value, inheritedValue.ToString()));
                    }
                } else {
                    var relativeInheritedValue = relativeAttribute.GetInheritValueAsObject();
                    if (relativeInheritedValue != null) {
                        finalExpression = Regex.Replace(finalExpression, c_RelativeInheritedPattern, m => string.Format("{0}{1}", m.Groups[1].Value, relativeInheritedValue.ToString()));
                    }
                }
            }
        ...
        public static (bool, string) RegexReplaceAttributesFromString(string expression, bool relative, IReadOnlyList<GroupCollection> matchedGroups, AttributeCollection attributeCollection, IReadOnlyDictionary<string, AttributeBase> otherAttributes, out string newStringExpression)
        {
            ...
            for (var i = 0; i < matchedGroups.Count; i++) {
                ...
                if (attribute != null) {
                    ...
                    var attributeValueText = attributeValue != null ? attributeValue.ToString() : "";
            ...
            out object evaluatedValue)
        {
            ...
            try { evaluatedValue = e.Evaluate(); } catch (Exception ex) {
